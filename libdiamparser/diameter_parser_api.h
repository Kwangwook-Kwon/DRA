...
/*                                                                        */
/* END_COPYRIGHT                                                          */
// $Id: diameter_parser_api.h,v 1.52 2005/09/06 16:18:31 vfajardo Exp $
...
public:
  ...
  inline T& operator=(T v) 
    { 
      AAA_VectorAttribute<T>::isSet = true; 
      (std::vector<T>&)(*this)=v; 
      return AAA_VectorAttribute<T>::value; 
    }
};
...
class AAA_MsgResultCode
{
    ...
    public:
       ...
       RCODE InterpretedResultCode() {
           ...
           if (code) {
               for (int i=RCODE_INFORMATIONAL;
                    i<=RCODE_PERMANENT_FAILURE;
                    i++) {
                   code -= 1000;
                   if ((code/1000) == 0) {
                       return RCODE(i);
...
class AAA_MsgDump {
   public:
      static void Dump(AAAMessage &msg) {
          ...
          AAA_LOG(LM_INFO, "     flags(r,p,e) = (%d,%d,%d)\n",
