...
/*                                                                        */
/* END_COPYRIGHT                                                          */
/* $Id: parser_avpvalue.h,v 1.28 2005/07/22 15:43:43 vfajardo Exp $ */
...
#else
  ACE_UINT64 ret;
  ACE_CDR::swap_8((const char*)&q,(char*)&ret);
  return ret;
#endif
}
      ...
      if (check(str.data(), str.size()) != 0)
		{
			...
			st.set(NORMAL, AAA_INVALID_AVP_VALUE, avp, getCodec()); 
			throw st;
		}
    }
 ...
 public:
  ...
  void parseRawToApp() throw(AAAErrorStatus)
      ...
      catch (AAAErrorStatus st)
	{
      ...
      grammar.uri.transport = AAA_PROTO_DIAMETER;
      if (!parse(str.begin(), str.end(), grammar, space_p).full)
      {
...
struct IPFilterRuleGrammar : public grammar<IPFilterRuleGrammar>
{
  ...
  struct definition
  {
	...
	[IntListFunctor(self.r.ipOptionList, 
			-AAA_IPFILTER_RULE_IP_OPTION_SSRR)] | 
	str_p("ssrr")
	[IntListFunctor(self.r.ipOptionList, 
			AAA_IPFILTER_RULE_IP_OPTION_SSRR)];
	...
	str_p("rr")
	[IntListFunctor(self.r.ipOptionList, AAA_IPFILTER_RULE_IP_OPTION_RR)];

      ipts = (ch_p('!') >> str_p("ts"))
	[IntListFunctor(self.r.ipOptionList, 
			-AAA_IPFILTER_RULE_IP_OPTION_TS)] |
	str_p("ts")
	[IntListFunctor(self.r.ipOptionList, AAA_IPFILTER_RULE_IP_OPTION_TS)];
	...
	[IntListFunctor(self.r.tcpOptionList, 
			-AAA_IPFILTER_RULE_TCP_OPTION_MSS)] | 
	str_p("mss")
	[IntListFunctor(self.r.tcpOptionList, 
			AAA_IPFILTER_RULE_TCP_OPTION_MSS)];
	...
	[IntListFunctor(self.r.tcpFlagList, 
			-AAA_IPFILTER_RULE_TCP_FLAG_FIN)] | 
	str_p("fin")
	[IntListFunctor(self.r.tcpFlagList, 
			AAA_IPFILTER_RULE_TCP_FLAG_FIN)];
      syn = (ch_p('!') >> str_p("syn"))
 ...
 public:
  ...
  void parseRawToApp() throw(AAAErrorStatus)
      ...
      catch (AAAErrorStatus st)
	{
      ...
      grammar.r.src.representation = AAA_IPFILTER_RULE_SRCDST_EXACT;
      grammar.r.dst.modifier = true;
      grammar.r.dst.representation = AAA_IPFILTER_RULE_SRCDST_EXACT;
	   ...
	   ->search(avp->avpCode, avp->vendorId)) == NULL)
	    {
	      ...
	      st.set(NORMAL, AAA_AVP_UNSUPPORTED, avp, getCodec());
	      throw st;
	    }
 ...
 public:
  ...
  void parseRawToApp() throw(AAAErrorStatus)
      ...
      catch (AAAErrorStatus st)
	{
      ...
      time = ntohl(*(diameter_time_t*)str.data());
  ...
  void parseAppToRaw() throw(AAAErrorStatus)
      ...
      if (e->dataType() != AAA_AVP_TIME_TYPE)
	{
      ...
      std::string& str = e2->dataRef(Type2Type<diameter_octetstring_t>());
      diameter_time_t& time = e->dataRef();
      ACE_UINT32 timeNBO; // time represented by Network Byte Order.
      timeNBO = ntohl(time);
