...
void AAA_ClientAuthSession::RxAnswer(std::auto_ptr<AAAMessage> msg) 
{
    ...
    m_Fsm.NotifyRx(AAA_SESSION_AUTH_EV_RX_SSAA, msg);
}
...
AAAReturnCode AAA_ClientAuthSession::RxDelivery(std::auto_ptr<AAAMessage> msg)
{
    ...
    RxHandler RxHandlerAry[] = {
        { AAA_ERR_INCOMPLETE, 0, false },
        { AAA_ERR_SUCCESS, AAA_SESSION_AUTH_EV_SSAA_OK, false },
        { AAA_ERR_NOSERVICE, AAA_SESSION_AUTH_EV_SSAA_NOSVC, true },
        { AAA_ERR_MSG_UNPROCESSED, AAA_SESSION_AUTH_EV_SSAA_ERROR, true },
        { AAA_ERR_FAILURE, AAA_SESSION_AUTH_EV_SSAA_FAIL, false }
    };
    Attributes().MsgIdRxMessage(*msg);
    AAAReturnCode rc = AnswerMsg(*msg);
    for (int i=0; i < sizeof(RxHandlerAry)/sizeof(RxHandler); i++) {
        if (RxHandlerAry[i].m_Rc == rc) {
            ...
            if (RxHandlerAry[i].m_Event != 0) {
                m_Fsm.Notify(RxHandlerAry[i].m_Event);
            }
            return (AAA_ERR_SUCCESS);
        }
    }
...
AAAReturnCode AAA_ClientAuthSession::Send(std::auto_ptr<AAAMessage> msg) 
{
    ...
    if ((m_Fsm.State() == AAA_SESSION_AUTH_ST_PENDING) &&
    ...
    m_Fsm.NotifyTx(AAA_SESSION_AUTH_EV_TX_SSAR, msg);
    return (AAA_ERR_SUCCESS);
}
...
AAAReturnCode AAA_ClientAuthSession::End()
{
    if (Attributes().AuthSessionState()() ==
        AAA_SESSION_STATE_MAINTAINED) {
        AAA_LOG(LM_INFO, "(%P|%t) Session state maintained, ignoring end command\n");
    }
    else if (m_Fsm.IsRunning()) {
        m_Fsm.Notify(AAA_SESSION_AUTH_EV_STOP);
    }
    m_Fsm.WaitOnReset();
