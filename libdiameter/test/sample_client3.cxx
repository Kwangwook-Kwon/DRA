                         ...
                         public AAA_SessionMsgMux<AAA_SampleClient>
{
    ...
    public:
        ...
        virtual AAAReturnCode RequestMsg(AAAMessage &msg) {
            ...
            // request messags. 
            AAA_LOG(LM_INFO, "(%P|%t) **** [Num:%d] Request message message received in client ??? ****\n", 
                    m_SessionNum);
...
class Client
{
    public:
        ...
        void Stop() {
           do {              
               std::cout << "Waiting till user is authorized and disconnected, #" 
                         << m_Client.SessionNum()
                         << std::endl;
               ACE_OS::sleep(1);
           } while (! (m_Client.UserAuthorized() &&
                       m_Client.SessionDisconnected()));
           m_Client.End();
        }
