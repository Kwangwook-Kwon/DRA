...
{        
    ...
    //
#if !defined(PANA_DEBUG)
    AddcardStateTableEntry(PANA_ST_CLOSED,  
                           PANA_ST_CLOSED);
#endif                           
...
class PANA_PsmRxPBA : public PANA_ServerRxStateFilter
{
   public:
      ...
      virtual void HandleMessage(PANA_Message &msg) {
          ...
          diameter_address_t *id = pacIdAvp.GetAvp(PANA_AVPNAME_DEVICEID);
...
void PANA_PaaSession::SendNotification(std::string &message)
{
    PANA_PaaEventVariable ev;
    m_PAA.LastTxNotification() = message;
    ev.Event_App(PANA_EV_APP_NOTIFICATION);
    Notify(ev.Get());
}
